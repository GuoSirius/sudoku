...
- 重排难度档位（中等→进阶，拉开轻松营与硬核营） ([214dc6c])
- 迷你导航铺满整屏且可横向滚动 + 提高中等/困难/专家/极限四档难度 ([690e875])
- 默认音量降至60% + 音量/音效开关并入设置同步(settings)，支持跨设备 ([91becd7])
- 摸鱼小窗自由跳页(mini-nav) + 音量滑块 + 提示/检查轻柔音 ([3c4c854])
- 全套游戏音效（落子/擦除/填错/通关）+ 设置开关与游戏内静音按钮 ([a2c98dd])
- 通关庆祝特效——全屏礼花 + 胜利弹窗弹出动画 ([3e81669])
- 落子后自动清除同行/列/宫中相同的候选数字 ([4700775])

### Bug Fixes

- 统一摸鱼模式下游戏顶栏暂停/音效按钮样式 ([293ce89])
- 挂起态先 resume 再调度，根治落子音时有时无 ([3c3a2d7])
- 游戏顶栏改两行布局，计时/统计不再被三图标挤压 ([f3767e6])
- 优化音效（全局手势预热防丢声 + 主增益节点提升清晰度） ([972ef4c])
- 摸鱼模式首页空白与导航条覆盖内容（home→menu 映射 + 按实测高度下移） ([2a2f7ac])

## [2.0.3] - 2026-08-17

### Features

- 复盘页刷新后自动续播到上次进度步 ([457ceb7])
- 所有页面支持刷新后停留（guide/replay 持久化恢复） ([0c219e8])
- 历史续玩保留计时、新增玩法指南页(L0-L9技巧)、强化固有数字高亮 ([bf5ca8b])
...
- 桌面端移除启动跳转线上站，解决白屏/无界面但进程常驻 ([9e06613])

## [2.0.1] - 2026-08-14

### Bug Fixes

- 新建游戏取消难度选择时保留原进行中对局 ([8a48961])

## [2.0.0] - 2026-08-14

### Features

- 每局技巧组合随盘面浮动，中等/困难可偶发 X-Wing/XY-Wing ([3eb8e53])
- 难度按 c+g 双轴均匀化（clues 主轴 + grade 封顶） ([1d03805])
- **grader:** 难度改为按逻辑技巧分级（方案B），逐级锻炼思维 ([f8394d1])
- **ui:** 选中错格锁红底 + 补齐 buildBoard 状态单测 ([7ea8e06])

### Bug Fixes

- 入门档保留更多提示(看着就简单)，消除留存隐患 ([2cd27b6])
- **ui:** 重做棋盘格子状态体系,统一优先级裁决 ([9f42053])
- **ui:** 修复行塌陷、同数高亮不全、首屏摸鱼入口闪烁 ([d5d4c44])

### UI

- 难度徽章改为「关键词·短技巧」组合（≤6 字） ([cc7b192])
- 简化难度展示为关键词，避免标题过长 ([3d6d87b])

### Code Refactoring

- 彻底移除底层 techniques 数据，清理 usedSet 等无用数据 ([a6e0b5e])
- 移除游戏头部「本局技巧」展示，修订难度体系注释与文档 ([6466dcb])

### Styles

- 难度选择弹窗按钮文本居中显示 ([4db62ef])

## [1.1.0] - 2026-08-13

### Features

- 移动端/PWA 屏蔽摸鱼入口(小窗按钮+老板键+伪装语言)，已开启的迷你窗/伪装屏仍生效 ([1caa0e2])
- **ui:** 排行榜前十名徽标配色(金/银/铜+7色相)，前三名整行同色强调 ([f145c94])
- **ui:** 摸鱼小窗按钮点击前二次确认，避免误触 ([df3b399])

### Bug Fixes

- 摸鱼入口判定纳入 isTauri/isCapacitor，精确覆盖桌面应用需/原生App不需 ([058cac3])
- 摸鱼入口改用 pointer:coarse+hover:none 判定触摸手持设备，移动端横竖屏均屏蔽 ([e7efa73])
- 摸鱼入口仅屏蔽小屏触摸设备，桌面浏览器与桌面PWA保留（修正误杀桌面PWA） ([81ac04d])
- **ui:** 修复“检查”二次误报无问题(改报当前错误总数)；排行榜改用卡片列表适配移动端 ([6141410])

### Chores

- **ci:** 临时停用 deploy.yml（Cloudflare 已直连仓库，推送即更新） ([928a309])

### Documentation

- README 新增「支持环境」小节，列出摸鱼入口的平台矩阵 ([d53d5f7])
- README 注明摸鱼小窗/老板键仅桌面端，移动端/PWA 自动隐藏入口 ([4361275])
- 依据全项目重写 README（补齐多端/云同步/发布流程），修正 tauri devUrl 端口 ([2e8be85])
- **test:** 摸鱼小窗二次确认补 DOM 测试(8 项)并更新 CHANGELOG/README ([398158f])

## [1.0.6] - 2026-08-13

### Features

- 摸鱼小窗按钮点击前加二次确认（「打开小窗」/「取消」），避免误触弹出独立窗口；DOM 测试新增 8 项确认交互断言 ([df3b399])
