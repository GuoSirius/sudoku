...
- **同一邮箱会自动合并为同一个账号**：Supabase 的「自动账号关联（automatic identity linking）」会在你用 GitHub 登录时，查找是否已存在相同且已验证的邮箱账号；若有，就把 GitHub 身份**关联到那个已有账号上**，两者共用同一个 `auth.users.id`（而不是新建第二个账号）。所以「用邮箱登」和「用 GitHub 登」在邮箱一致时会是**同一个用户**。
- **前提条件**：① 两种方式用的是**同一个邮箱**；② 邮箱必须是**已验证**状态（邮箱魔法链接点开即验证；GitHub 侧只要 GitHub 给了已验证邮箱即可）。
- 云端数据按 `auth.uid()`（用户唯一 ID）隔离——合并后两种登录方式 `uid` 相同，自然读写**同一行** `user_data`，数据天然统一，无需额外代码。
- 代码上：邮箱是默认主入口；GitHub 作为「可选」按钮，**只有当你配了 GitHub 且打开开关时才显示**，没配就自动隐藏，不影响邮箱登录。

> ⚠️ **GitHub 邮箱隐私的坑**：GitHub 默认可能把邮箱设为「private」，此时 Supabase 从 GitHub 拿到的邮箱是 `noreply@users.noreply.github.com`（或拿不到邮箱），和你的真实邮箱不一致 → **自动合并失败，变成两个独立账号**。要合并成功，请在 GitHub → Settings → Emails 里**取消勾选「Keep my email addresses private」**（或确保主邮箱公开）。这是我们这套同步里让两种方式合并的唯一前提。
