...
| --- | --- | --- |
| 选中格子 | 点击格子 | 方向键移动选中 |
| 记候选数 | 点数字盘 1–9（**单击**切换添加/取消） | PC 按住 `Ctrl` + 数字键 |
| 填入数字 | 点数字盘 1–9（**双击**填入） | 数字键 `1`–`9`（需先选中格子） |
| 擦除 | 点「擦除」 | `Backspace` / `Delete` / `0` |
| 笔记模式 | 点「笔记」切换（影响键盘输入；数字盘默认单击即记候选） | 按 `N` 切换 |
| 提示（填正确值） | 点「提示」 | — |
| 暂停 / 继续 | 顶栏 ⏸（可切换暂停/继续）/ 暂停横幅「继续」；游戏中点 ☰ 会先暂停再回首页 | — |

- **笔记（候选数）**：数字盘**单击**任意数字即把该数字作为候选（小字）记入选中格，再次单击同一数字则取消；同一格可记多个候选。无论是否开启「笔记」模式，数字盘单击都记候选。
- **取消单个笔记**：在格子里点一下笔记小数字即可删除该候选；或用数字盘再点同一数字切换取消；「擦除」键会清空整格笔记。
- **填入数字**：数字盘**双击**某数字即把该数字正式填入选中格（单击是记候选）；键盘直接按数字键也是填入。
