...
核心玩法（计时、个人排行榜、历史复盘、中途暂存续玩、六档技巧难度、笔记、提示、老板键伪装、摸鱼小窗）三端共用同一套 `web/` 代码，单一来源，无需双份维护。
...
- 🎚 **六档技巧难度**：入门 / 简单 / 中等 / 困难 / 专家 / 极限。难度由「给定数（clues）主轴 + 技巧层级（grade）浮动」共同决定：clues 把入门→极限单调铺开、保留存；grade 随盘面自然浮动、不写死，同一档每局锻炼的技巧组合都不同（中等/困难偶发触达 X-Wing / XY-Wing）；生成始终保证唯一解
...
- 🐟 **摸鱼小窗**：独立迷你窗口（只显示棋盘、强制暗色），可拖到屏幕角落；点击按钮需二次确认防误触。电脑上"不便大屏操作"时正需要它——**桌面浏览器与桌面 PWA 均可用**；触摸手持设备（手机 / 平板，横竖屏都算）自动隐藏入口（无独立可拖拽窗口、无实体老板键）
- 🕶 **老板键伪装**：按 ` 键（桌面端全局 `Alt + \`` 在窗口失焦时也能触发）在「游戏」与「伪装工作界面」间瞬间切换；伪装可选前端 / PHP / Java / Python。同样仅桌面端有意义，触摸手持设备屏蔽入口
...
数独核心玩法（计时、排行、复盘、云同步、全球榜等）三端通用。但**摸鱼（摸鱼小窗 + 老板键伪装）是桌面端专属能力**——需要可拖拽的独立窗口与实体键盘，移动端 / 原生 App 没有意义，入口会自动隐藏（已开启的迷你窗 `?mini=1` 与伪装屏 `.boss` 仍照常生效）。

| 运行环境 | 摸鱼入口 | 说明 |
| --- | --- | --- |
| PC 端浏览器（Win / Mac / Linux） | ✅ 开启 | 电脑操作，"不便大屏"时正需要 |
| 桌面应用（Tauri） | ✅ 开启 | 即使 Windows 触屏平板模式也保留（`isTauri` 兜底全局快捷键） |
| 桌面 PWA（装成独立窗口） | ✅ 开启 | 本质仍是电脑 |
| 触摸笔记本（带键鼠的 Windows 触屏本） | ✅ 开启 | 有实体键盘、可开窗口 |
| 移动端浏览器（手机） | ❌ 隐藏 | 无独立窗口、无实体老板键 |
| 移动端 PWA（手机 standalone） | ❌ 隐藏 | 同上 |
| 原生 App（Capacitor 安卓 / iOS） | ❌ 隐藏 | App 即本体，无需伪装 / 小窗 |
| 平板（iPad / 安卓平板，浏览器或 PWA） | ❌ 隐藏 | 触摸手持设备，横竖屏都算 |

> 判定式：`SLACK_ENABLED = isTauri() || (!IS_TOUCH_DEVICE && !isCapacitor())`。
