...
| --- | --- | --- |
| 选中格子 | 点击格子 | 方向键移动选中 |
| 记候选数 | 点数字盘 1–9（**单击**切换添加/取消）；点格子里的候选数字（**单击**切换） | PC 按住 `Ctrl` + 数字键 |
| 填入数字 | 点数字盘 1–9（**双击**填入）；**双击**格子里的候选数字即把该候选填入 | 数字键 `1`–`9`（需先选中格子） |
...
- **格子里的候选（单击 / 双击）**：对于已经记了候选的格子，**单击**其中的某个候选小数字 = 切换该候选（添加或取消）；**双击**某个候选小数字 = 把它正式填入该格并清空其余候选（即「笔记转正」）。这与数字盘的「单击记候选 / 双击填入」完全一致。
- **取消单个笔记**：在格子里**单击**某个候选小数字即可切换它（添加或取消）；或用数字盘再点同一数字切换取消；「擦除」键会清空整格笔记。
- **填入数字**：数字盘**双击**某数字即把该数字正式填入选中格（单击是记候选）；**双击**格子里的候选小数字同样填入该格；键盘直接按数字键也是填入。
- **笔记模式（仅影响键盘）**：开启后键盘数字键改为记候选；数字盘与单元格候选单击均默认记候选，**无需为记候选而切模式**。PC 端按住 `Ctrl` 再按数字键，等价于临时开启笔记模式记候选。
