...
- 🐟 **摸鱼小窗**：独立迷你窗口（只显示棋盘、强制暗色），可拖到屏幕角落；点击按钮需二次确认防误触。**仅桌面端可用**，移动端 / PWA 安装态自动隐藏入口（无独立可拖拽窗口、无实体老板键）
- 🕶 **老板键伪装**：按 ` 键（桌面端全局 `Alt + \`` 在窗口失焦时也能触发）在「游戏」与「伪装工作界面」间瞬间切换；伪装可选前端 / PHP / Java / Python。同样仅桌面端有意义，移动端 / PWA 屏蔽入口
