...
- 🔁 **未完成可续玩**：历史中「未完成」的记录提供「继续」——把该局恢复为当前对局接着玩；「已完成」记录仅提供「复盘」
...
| 笔记模式 | 点「笔记」切换（决定数字盘/键盘默认是「记候选」还是「回填」） | 按 `N` 切换 |
| 提示（填正确值） | 点「提示」 | — |
| 暂停 / 继续 | 顶栏 ⏸（可切换暂停/继续）/ 暂停横幅「继续」；游戏中点 ☰ 会先暂停再回首页 | — |

- **统一判定 `wantNote = 笔记模式 || 按住 Ctrl`**：数字盘与键盘数字输入，只要满足其一就「记候选（切换：有则删、无则加）」，**否则回填**。移动端无 Ctrl，`wantNote` 仅由「笔记」按钮决定——普通模式点数字盘=回填、笔记模式点数字盘=记候选，逻辑与 PC 完全一致，零额外心智负担。
- **数字盘（1–9）**：
  - 普通模式单击 = 回填；笔记模式 / `Ctrl` 单击 = 记候选；**双击 = 强制回填**（笔记模式下也能从数字盘直接填入）。
  - 键盘：普通模式按数字键 = 回填；笔记模式 / `Ctrl`+数字键 = 记候选。
- **格子里的候选（单击 / 双击）**：已记候选的格子，**单击**某个候选小数字 = 切换该候选（添加或取消，与模式无关）；**双击**某个候选小数字 = 把它正式填入该格并清空其余候选（即「笔记转正」）。
- **取消单个笔记**：在格子里**单击**某个候选小数字即可切换（添加或取消）；数字盘在笔记模式 / `Ctrl` 下再点同一数字也可切换取消；「擦除」会清空整格笔记。
- **PC 组合键**：按住 `Ctrl` 时，单击数字盘或按数字键都临时走「记候选」，免频繁切笔记模式；松开 `Ctrl` 复位。
- **提示**会判定为「使用提示一次」，并计入复盘与排行榜的提示数。
- 选中某格后，同行 / 同列 / 同宫与相同数字会高亮，冲突与错误标红。
- 未选中格子时按数字键只会提示「请先选择一个格子」，不会自动填入，避免误填。
- **暂停（非阻塞）**：点顶栏 ⏸ 进入暂停——**仍停留在游戏界面**，棋盘轻微变暗提示、计时停止、进度自动保存。此时可点「退出到首页」或顶栏 ☰ 去做别的事（点 ☰ 会先自动暂停，计时不会偷偷继续）；之后在首页点「继续游戏」或返回游戏点「继续」即可续玩。
- **错误提示（不泄题）**：默认「仅冲突」——只把同行/列/宫里**重复**的数字标红，**不**逐格比对答案，因此不会削弱难度（不会变相告诉你正确答案）。需要更强反馈可在设置里切到「全量核对」（即时比对答案并计错），或选「关闭」彻底不标红。任何时候都能点工具栏「🔍 检查」**手动**揭示当前错误并计错——游戏中零自动泄题，想核对时再核对。
